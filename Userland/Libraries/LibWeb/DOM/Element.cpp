...
StringView Element::lang() const
{
    // 1. If the node is an element that has a lang attribute in the XML namespace set
    //      Use the value of that attribute.
    auto const maybe_xml_lang = get_attribute_ns(Namespace::XML, HTML::AttributeNames::lang);
    if (maybe_xml_lang.has_value())
        return maybe_xml_lang.value();
    ...
    if (is_html_element() || namespace_uri() == Namespace::SVG) {
        auto const maybe_lang = get_attribute(HTML::AttributeNames::lang);
        if (maybe_lang.has_value())
            return maybe_lang.value();
    }
